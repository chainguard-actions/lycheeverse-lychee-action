...
# Tokenize ARGS (a whitespace-separated list of arguments) into an array,
# honoring quotes and backslashes the same way the shell would.
ARGS_ARRAY=()
if [ -n "$ARGS" ]; then
  while IFS= read -r -d '' t; do ARGS_ARRAY+=("$t"); done \
    < <(printf '%s' "$ARGS" | xargs printf '%s\0')
fi
